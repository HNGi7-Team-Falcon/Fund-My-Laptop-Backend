  ...
  if (payment.data.status === "error") {
    ...
    res
      ...
      .send(
        response(
          "Valid Card",
          { flw_ref: payment.data.data.flw_ref },
          "Successful"
        )
      );
  }
    ...
    res
      ...
      .send(
        response(
          ...
          { id: validate_payment.data.data.id },
          "successful"
        )
      );
  } else {
    ...
    res
      ...
      .send(
        response("Transaction Successful", {
          Transaction_id: verify_payment.data.data.id,
        })
      );
  } else {
    throw new CustomError(
